  ...
  return (
    <div className="app">
      <header className="app-header">
        <h1>Pool Manager</h1>
        <p>Open a pool to review backend records, fix highlighted errors, or upload Excel.</p>
      </header>
      ...
      <div className="pool-list">
        {POOL_TEMPLATES.map((pool) => {
          ...
          const issueCount = records.filter(
            (r) => (r.recordStatus ?? '') === 'failed' || (r.errorKeys ?? []).length,
          ).length

          return (
            <Link key={pool.id} to={`/pools/${pool.id}`} className="pool-list-card">
