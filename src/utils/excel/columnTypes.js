 ...
 */
export function resolveColumnType(column) {
  const explicit = String(column?.type ?? '')
    .trim()
    .toLowerCase()

  if (explicit && SUPPORTED_COLUMN_TYPES.includes(explicit)) {
    return explicit
  }
...
export function isSupportedColumnType(type) {
  return SUPPORTED_COLUMN_TYPES.includes(String(type ?? '').toLowerCase())
}
